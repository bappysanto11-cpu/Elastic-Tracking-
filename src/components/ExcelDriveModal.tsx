...
export const ExcelDriveModal: React.FC<ExcelDriveModalProps> = ({
  ...
  const [sharedLinkInput, setSharedLinkInput] = useState<string>('');
  ...
  const handleOpenOneDriveWeb = () => {
    // Export file first for user convenience
    exportPackingSheetToExcel(sheetData, summary);
    // Open Microsoft OneDrive / Excel Online
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
          ...
          {activeTab === 'onedrive' && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="border border-sky-200 bg-gradient-to-br from-sky-50/50 to-white rounded-xl p-4 flex flex-col justify-between hover:border-sky-300 transition">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 px-3 bg-sky-600 hover:bg-sky-700 text-white text-xs font-bold rounded-lg shadow-xs transition cursor-pointer"
                  >
                ...
                {/* 2. Excel Online */}
                <div className="border border-emerald-200 bg-gradient-to-br from-emerald-50/50 to-white rounded-xl p-4 flex flex-col justify-between hover:border-emerald-300 transition">
                  <div className="flex items-start gap-3 mb-3">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg shadow-xs transition cursor-pointer"
                  >
              ...
              {/* Google Drive Option */}
              <div className="flex items-center justify-between p-3 border border-slate-200 rounded-xl bg-white text-xs">
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7 rounded-lg bg-amber-100 text-amber-700 flex items-center justify-center font-bold">
                    G
                  </div>
                  <div>
                    <span className="font-bold text-slate-900">Google Drive & Google Sheets</span>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg border border-slate-300 hover:bg-slate-100 text-slate-700 font-semibold flex items-center gap-1 cursor-pointer"
                >
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Supports Microsoft Excel (<code className="font-mono text-slate-700 font-bold">.xlsx</code>, <code className="font-mono text-slate-700 font-bold">.xls</code>) & CSV
                  </p>
                </div>
                <button
                  type="button"
                  className="mt-2 px-4 py-1.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold rounded-lg shadow-xs"
                >
              ...
              {previewData && (
                <div className="border border-slate-200 rounded-xl p-3.5 bg-white space-y-3">
                  ...
                  {previewData.header && (
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] bg-slate-50 p-2 rounded-lg border border-slate-200">
                      <div>
                        <span className="text-slate-500 block">REF:</span>
                        <span className="font-bold font-mono text-slate-900">{previewData.header.ref || sheetData.ref}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 block">Buyer:</span>
                        <span className="font-bold text-slate-900">{previewData.header.buyer || sheetData.buyer}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 block">Customer:</span>
                        <span className="font-bold text-slate-900">{previewData.header.customer || sheetData.customer}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 block">Unit Wt:</span>
                        <span className="font-bold font-mono text-slate-900">{previewData.header.defaultWtPerUnit || sheetData.defaultWtPerUnit} gm</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex justify-end gap-2 pt-1">
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-slate-900 border border-slate-300 rounded-lg"
                    >
                      {lang === 'en' ? 'Cancel' : 'বাতিল'}
                    </button>
                    <button
                      onClick={handleConfirmImport}
                      className="px-4 py-1.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-sm flex items-center gap-1.5 cursor-pointer"
                    >
            ...
            <div className="space-y-4">
              <div className="border border-slate-200 rounded-xl p-4 bg-slate-50 space-y-3">
                ...
                <div className="text-xs text-slate-600 space-y-1 bg-white p-3 rounded-lg border border-slate-200">
                  <p className="font-semibold text-slate-900">Included Sheets in Workbook:</p>
                  <div className="flex items-center gap-2 font-mono text-[11px] text-slate-700">
                    <span className="px-2 py-0.5 bg-slate-100 rounded border">1. Packing List</span>
                    <span className="px-2 py-0.5 bg-slate-100 rounded border">2. Net Weight Summary</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg shadow-sm transition cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 py-2.5 px-4 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold rounded-lg shadow-sm transition cursor-pointer"
                  >
        ...
        <div className="px-5 py-3 border-t border-slate-200 bg-slate-50 flex items-center justify-between text-xs text-slate-500">
          ...
          <button
            onClick={onClose}
            className="px-3 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-800 font-semibold rounded-lg transition cursor-pointer"
          >
